...
2. `resolve_llm_config()` — which model (and model source) backs each
   of `aiform/llm.py`'s two roles (`implementation`, `review`), from a
   **separate, non-secret** `.aiform/config.yaml` file. Deliberately not
   the same file or resolution order as credentials: a model name isn't
   a secret, so there's no shell-history/echo concern motivating a
   hand-edit-only fallback file here, and this file is safe to have a
   working default with no user setup at all.
...
- File present but only overriding some fields (e.g. just
  `llm.review.model`) → the omitted fields keep their default values;
  this is a shallow merge over `DEFAULT_LLM_CONFIG`, not a
  replace-the-whole-`llm.review`-object-if-any-key-is-present merge —
  setting `llm.review.model` alone must not silently reset
  `llm.review.source` to some other value.
...
- A top-level value, `llm:`, `llm.implementation`, or `llm.review` that
  parses to something other than a YAML mapping (e.g. `llm: anthropic`,
...
- **Per-resource-kind LLM configuration** — `LLMConfig` has exactly two
  global roles (`implementation`, `review`); per-resource-kind overrides
  are real future work, not built now (see `specs/llm.md`'s Out of
  scope).
