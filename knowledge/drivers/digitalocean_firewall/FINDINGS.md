...
`knowledge/` only on a **second** observation, in another resource or
another provider — one observation is an anecdote.

## Promotion candidates

| Finding | Would become | Evidence | Held because |
|---|---|---|---|
| DigitalOcean coerces scalar types on store rather than rejecting (int port → `"22"`, `"TCP"` → `"tcp"`, `"all"` → `"0"`) | `csp/digitalocean/scalar-coercion` | `04`, `05`, `10` | seen in one resource only |
| A server-added field absent from the published OpenAPI schema is a phantom-diff source | `driver/server-added-fields` | `02`, `03` | seen once; needs a second provider |
| To learn whether an omitted field is *reset* or *left alone*, the resource must first hold a non-empty value | `probing/reset-vs-unchanged` | `24`, `25`, `26` | method finding; strongest candidate |
